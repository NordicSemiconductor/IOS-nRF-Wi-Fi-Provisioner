...
final class ProvisionerInfoTests: XCTestCase {
    ...
    func testNotConnectedThrowing() {
        let failedProvisioner = DeviceManager(deviceId: noServiceDevice.identifier)
    ...
    func testEmptyData() throws {
        let failedProvisioner = DeviceManager(deviceId: badVersionDataDevice.identifier)
        failedProvisioner.infoDelegate = infoDelegate
        failedProvisioner.connect()
    ...
    func testInvalidArgument() throws {
        let provisioner = DeviceManager(deviceId: invalidArgumentDevice.identifier)
        provisioner.infoDelegate = infoDelegate
        provisioner.connect()
    ...
    func testInvalidProto() throws {
        let provisioner = DeviceManager(deviceId: invalidProtoDevice.identifier)
        provisioner.infoDelegate = infoDelegate
        provisioner.connect()
    ...
    func testInternalError() throws {
        let provisioner = DeviceManager(deviceId: internalErrorDevice.identifier)
        provisioner.infoDelegate = infoDelegate
        provisioner.connect()
    ...
    func testSucceedStatus() throws {
        ...
        case .success(let status):
            ...
            /*
             ...
             auth: .open
             */
            /// Look at mock in ``Provisioner/Tests/ProvisionerTests/Provisioner/ProvisionerInfoTests.swift``
            ...
            /*
             Look at mock in ``Tests/ProvisionerTests/Mock/Model+Extensions/ScanParams+Ext.swift:12``
             band: .band5Gh,
             passive: true,
             periodMs: 100,
             groupChannels: 1
